...
Similarly fill in the empty destination values in the Threads data frame.
```{r da2.1}
index = (Threads$ToType=="")
Threads$To[index] <- "All Network"
Threads$ToId[index] <- "company"
Threads$ToType[index] <- "user"

index = (Threads$ToType==NA)
Threads$To[index] <- "All Network"
Threads$ToId[index] <- "company"
Threads$ToType[index] <- "user"
```

Merge the two data frames into one using *rbind*.
```{r da2.2}
Messages <- rbind(Threads,Replies)
```
...
```
Make sure that we have a repeatable pseudo-random sequence by assigning a set seed.
```{r da4}
set.seed(123345)
```

"Anonymize" the names, by selecting creating a translation table of real to randomnames and replacing the real names with the newly generated random names in the *From* and *To* columns of the Messages data frame.
```{r da5}
...
a more detailed account of the different summaries is given in [@jurka2011].
```{r tm5}
...
Save the document summary to file.
```{r tm7, prompt=FALSE}
write.csv(analytics@document_summary, "data/DocumentSummary.csv")
```
