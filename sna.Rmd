...
Let's look at the first five rows and only the *From*, *To*, and *Message* columns from the *Messages* data frame.
```{r de1, results="asis"}
# switch comments if generating pdf
 print(xtable(Messages[1:5,c("From","To","Message")],tabular.environment='longtable'), 
...
```{r de2,  fig.title="Heatmap of messages between individuals and groups in the social network "}
base_size <- 10
heat_map <- (  ggplot(Messages, aes(From,To)) 
...
```{r de2,  fig.title="Messages sent by individuals in the social network "}
plot_top <- (ggplot(Messages, aes(x=From)) + geom_bar() + labs(x = "", y = "")
  + theme_grey(base_size = base_size)
  + theme(
       legend.position = "none"
     , axis.ticks = element_blank()
...
Suimilarly, who receives most messages. For individuals it may mean that they are valuable members of the community, depending on context.
```{r de2,  fig.title="Messages sent to individuals and groups in the social network "}
plot_right <- (ggplot(Messages, aes(x=To)) + geom_bar() + labs(x = "", y = "") 
  + theme_grey(base_size = base_size)
  + theme(
       legend.position = "none"
     , axis.ticks = element_blank()
...
grid.arrange(plot_top, plot_blank, heat_map, plot_right, ncol=2, nrow=2, widths = c(3,2), heights = c(2,3))
