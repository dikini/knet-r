...
```{r de2,  fig.star = TRUE, fig.title="Counts of messages from, between, and to individuals in the social network "}

#count the number of messages between all pairs of people
mcount <- ddply(Messages,.(From,To),nrow)
colnames(mcount) <- c("From", "To", "Count")

base_size <- 10

plot_blank <- ggplot(Messages, aes(From,To)) + geom_blank() + theme(legend.text = element_blank(), axis.text=element_blank(), axis.title = element_blank(), axis.ticks=element_blank(),  panel.background = element_blank())

heat_map <- (  ggplot(mcount, aes(From,To)) 
...
plot_right <- (ggplot(Messages, aes(x=To)) + geom_bar() + labs(x = "", y = "") + coord_flip()
  + theme_grey(base_size = base_size)
  + theme(
       legend.position = "none"
     , axis.ticks = element_blank()
