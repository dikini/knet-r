...
```{r de1, results="asis", fig.cap="Extract from the messages data", message=FALSE,warnings = FALSE}
Messages$Text <- strtrim(Messages$Message,30)
# switch comments if generating pdf
# print(xtable(Messages[1:5,c("From","To","Text")],tabular.environment='longtable'), main="A slice of data from the Messages frame")
kable(Messages[1:5,c("From","To","Text")],format = 'markdown')
...
```{r tm1 ,eval=FALSE}
doc_matrix <- create_matrix(cbind(Messages["ThreadId"],Messages["From"],Messages["To"], Messages["Text"]), language="english", removeNumbers=FALSE, stemWords=TRUE, weighting=tm::weightTfIdf)

## nodet: replace Messages$FromId with Code columns
container <- create_container(doc_matrix, Messages$FromId,trainSize=1:60, testSize=61:80, virgin=FALSE)
...
```{r  fig8, fig.cap="First clique in a separate graph"}
...
hist(grc$membership, main="")
```

```{r  fig11, fig.cap="The graph with the communities coloured in different colours"}
...
```{r  fig14,  fig.cap="Histogram of the fastgreedy community membership"}
hist(gfc$membership, main="")
```
a
```{r  fig15, fig.cap="The graph with the communities coloured in different colours"}
plot(gfc,gr, vertex.label=NA, vertex.size=5)
```
```{r  fig16, fig.width=10, fig.height=8, fig.cap="The dendogram of the community hierarchy"}
dendPlot(gfc)
```
